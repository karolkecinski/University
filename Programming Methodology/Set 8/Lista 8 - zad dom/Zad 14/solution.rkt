...
; Authors : Jakub Świgło || Jakub Bok || Karol Kęciński
...
(struct apply-expr (f list)   #:transparent)
  ...
  (cond
    ...
    [(and (list? q) (eq? (length q) 4) (eq? (first q) 'if))
     (if-expr (parse (second q))
              (parse (third q))
              (parse (fourth q)))]
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'apply))      ;Warunek dla apply,
     (apply-expr (parse (second q)) (parse (third q)))]             ;zamiana na listę cons
...
(define (list->cons list)
  (if (null? list)
      (null-expr)
      (cons-expr (parse (car list)) (list->cons (cdr list)))))
...
(define (eval-env e env)
  (match e
    ...
    [(apply-expr f list)
     (eval-env (decompose f (reverse (eval-env list env))) env)]
    [(lam x e) (clo x e env)] ; <--------------------------------- !!!
    [(app f e) ; <------------------------------------------------ !!!
...
(define (decompose f list)
  (if (null? (cdr list))
      (app f (const (car list)))
      (app (decompose f (cdr list)) (const (car list)))))
