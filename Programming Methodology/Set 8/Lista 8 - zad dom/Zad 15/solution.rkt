...
; Authors : Karol Kęciński || Jakub Świgło || Jakub Bok
...
(define (lazy-eval e env)                ;Tworzymy drugą funkcję, która jako pierwsza obliczy leniwie
  (match e                               
    [(const n) (const n)]
    [(binop op l r) (binop op (lazy-eval l env)
                                   (lazy-eval r env))]
    [(let-expr x e1 e2)
     (lazy-eval e2 (env-add x (lazy-eval e1 env) env))]
    [(var-expr x) (env-lookup x env)]
    [(if-expr eb et ef) (if (lazy-eval eb env)
                            (lazy-eval et env)
                            (lazy-eval ef env))]
    [(cons-expr e1 e2) (cons (lazy-eval e1 env)
                             (lazy-eval e2 env))]
    [(car-expr e) (car (lazy-eval e env))]
    [(cdr-expr e) (cdr (lazy-eval e env))]
    [(null-expr) null]
    [(null?-expr e) (null? (lazy-eval e env))]
    [(lam x e) (clo x e env)] ; <--------------------------------- !!!
    [(app f e) ; <------------------------------------------------ !!!
     (let ([vf (lazy-eval f env)]
           [ve (lazy-eval e env)])
       (match vf [(clo x body fun-env)
                  (lazy-eval body (env-add x ve fun-env))]))]))

(define (eval-env e env)     ;Wszystko co możemy obliczamy leniwie, następnie przekazujemy do eval-env, który zwraca wynik
  (match e                              
    [(const n) n]
...
(define (eval e) (eval-env (lazy-eval e env-empty) env-empty))
