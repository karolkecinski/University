...
; Do letrec.rkt dodajemy stringi
;
...
(define (EVAL p)
  `(let [op->proc (lambda (op)
                    (if (eq? op "+") (lambda (x y) (+ x y))
                    (if (eq? op "*") (lambda (x y) (* x y))
                    (if (eq? op "-") (lambda (x y) (- x y))
                    false))))]
   (letrec [eval (lambda (e)
                   (if (eq? (car e) "binop")
                       (let [op (car (cdr e))]
                       (let [l  (car (cdr (cdr e)))]
                       (let [r  (car (cdr (cdr (cdr e))))]
                       (op->proc op (eval l) (eval r)))))
                   (if (eq? (car e) "const")
                       (car (cdr e))
                   false)))]
   (eval ,p))))
...
(define EXPRESSION
   '(let [const (lambda (n) (cons "const" (cons n null)))]
    (let [binop (lambda (op l r)(cons "binop" (cons op (cons l (cons r null)))))]
    (binop "+" (const 2) (binop "*" (const 2) (const 2))))))
